    ...
    function MainController($scope, $timeout, $window, $firebaseArray, FireHangman) {
	    ...
	    vm.addMessage = function(evt) {
	    	...
	    	var _message = {
                ...
                isStart = new RegExp('\/start', 'i'),
                isGuess = new RegExp('\/guess', 'i');

            if(isStart.test(vm.message)) {
                
                //Start Game
                FireHangman.start();

            } else if(isGuess.test(vm.message)) {

                // Looks like we're gussing a letter
                var letter = vm.message.slice(-1);
        ...
        function getUserId() {
            ...
            if($window.localStorage.getItem('fireHangman_userId') === null){
                ...
                _id = $window.localStorage.getItem('fireHangman_userId');
            }      
